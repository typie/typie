...
export default class AbstractHastePackage {
    ...
    public activate(pkgList: string[], item: HasteRowItem, callback: (data) => void) {
        console.warn('No override "activate" method found in ' + this.packageName);
    }

    public enterPkg(pkgList: string[], item: HasteRowItem, callback: (data) => void) {
        console.warn('No override "enterPkg" method found in ' + this.packageName);
        this.getFirstRecords(10);
    }

    public clear(pkgList: string[], callback: (data) => void) {
        console.warn('No override "clear" method found in ' + this.packageName);
        this.getFirstRecords(10);
    }

    public remove(pkgList: string[], item: HasteRowItem, callback: (data) => void) {
        console.warn('No override "remove" method found in ' + this.packageName);
    }
    ...
    public getFirstRecords(numOfRecords: number = 10) {
        this.haste.getRows(numOfRecords).orderBy("count").desc().go()
