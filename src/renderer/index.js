...
cssLink.setAttribute('href', "themes/default/style.css");
...
function init(){
    ...
    let mainVue = new Vue({
        ...
        watch: {
            'searchVal': function (val, oldVal) {
                ...
                this.searchIt(val, this.currentPackage);
            },
        ...
        methods: {
            ...
            searchIt: (val, currentPackage) => {
                if (val !== '') {
                    // this.selected = null
                    ipcRenderer.send('search', {value: val, currentPackage: currentPackage});
                } else {
                    this.refreshList();
                }
            },
        ...
        <ul class="results" v-bind:class="">
            <li v-for="(item, index) in list" v-bind:class="{selected: selected===index}" v-on:click="click(item)">
        ...
        <div class="footer">
            ...
            <div class="status"><span v-if="workers > 0"> items in Catalog · Haste 1.0</span>
            </div>
        </div>
        </div>`
    ...
    ipcRenderer.on('resultList', (event, list) => {
        mainVue.list = [];
        Object.keys(list).forEach(function(key) {
            mainVue.list.push(list[key]);
        });
        console.log(mainVue.list);
